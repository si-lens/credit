...
RSpec.describe Credit::CardValidators::CalculateChecksum do
  describe "#call" do
    context "When calling for valid card number" do
      it "Returns true" do
        ...
        card_numbers.each do |card_number|
          expect(described_class.new(card_number).call).to eq(true)
        end
      end
    end
    context "When calling for invalid card number" do
      it "Returns false for invalid card number" do
        ...
        card_numbers.each do |card_number|
          expect(described_class.new(card_number).call).to eq(false)
        end
      end
    end
  end
